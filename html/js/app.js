            ...
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "twitter") {
                    if (QB.Phone.Data.IsOpen) {
                        $.post('https://ks-phone/GetTweets', JSON.stringify({}), function(Tweets){
                    ...
                    $.post('https://ks-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('https://ks-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('https://ks-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('https://ks-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('https://ks-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('https://ks-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('https://ks-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('https://ks-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('https://ks-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });
                } else if (PressedApplication == "houses") {
                    $.post('https://ks-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('https://ks-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    $.post('https://ks-phone/GetAvailableTaxiDrivers', JSON.stringify({}), function(data){
                        SetupTaxiDrivers(data);
                    });
                }
                else if (PressedApplication == "gallery") {
                    $.post('https://ks-phone/GetGalleryData', JSON.stringify({}), function(data){
                    ...
                    $.post('https://ks-phone/GetJobs', JSON.stringify({}), function(data){
                        LoadEmploymentApp(data)
                    });
                } else if (PressedApplication == "debt") {
                    $.post('https://ks-phone/GetPlayersDebt', JSON.stringify({}), function(data){
                        LoadDebtJob(data);
                    });
                }
                else if (PressedApplication == "gopro") {
                    $.post('https://ks-phone/SetupGoPros', JSON.stringify({}), function(Cams){
                        SetupGoPros(Cams);
                    })
                }
                    ...
                    $.post('https://ks-phone/GetChatRooms', JSON.stringify({}), function(ChatRooms){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://ks-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
$(document).on('click', '.phone-take-camera-button', function(event){
    ...
    $.post('https://ks-phone/TakePhoto', JSON.stringify({}),function(url){
        // setUpCameraApp(url)
    })
...
$(document).on('click', '.phone-silent-button', function(event){
    event.preventDefault();
    $.post('https://ks-phone/phone-silent-button', JSON.stringify({}),function(Data){
        if(Data){
...
QB.Phone.Notifications.Custom.Add = function(icon, title, text, color, timeout, accept, deny) {
    $.post('https://ks-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
...
$(document).on('click', ".notification-accept", function() {
    $.post('https://ks-phone/AcceptNotification', JSON.stringify({})),
    QB.Phone.Animations.TopSlideUp(".phone-notification-container-new", 150, -8);
    QB.Phone.Notifications.Timeout = null
...
$(document).on('click', ".notification-deny", function() {
    $.post('https://ks-phone/DenyNotification', JSON.stringify({})),
    QB.Phone.Notifications.Timeout = null
