...
RegisterNetEvent('qb-phone:server:SendEmploymentPayment', function(Job, CID, amount)
    ...
    if Config.RenewedBanking then
        if not exports['ks-pank']:removeAccountMoney(Job, amt) then return notifyPlayer(src, "Insufficient Funds...") end
        ...
        local trans = exports['ks-pank']:handleTransaction(Job, title, amt, "Payment given of $"..amt.." given to "..RecieverName, BusinessName, RecieverName, "withdraw")

        ---- Player Account ----
        exports['ks-pank']:handleTransaction(Reciever.PlayerData.citizenid, title, amt, "Payment recieved of $"..amt.." recieved from Business "..QBCore.Shared.Jobs[Job].label.. " and Manager "..BusinessName, BusinessName, RecieverName, "deposit", trans.trans_id)
    else
        if not exports['ks-management']:RemoveMoney(Job, amt) then return notifyPlayer(src, "Insufficient Funds...") end
    end
    ...
    if paid then
        if amount == bills[source].amount then
            ...
            if Config.RenewedBanking then
                ...
                local trans = exports['ks-pank']:handleTransaction(cid, "Personal // Invoice Transaction", amount, text, bills[source].name, name, "withdraw")

                local text2 = "A invoice issued by "..bills[source].name.." Worth $"..amount.." was paid off by "..name.." for "..bills[source].notes
                exports['ks-pank']:handleTransaction(bills[source].job, "Business // Invoice Transaction", amount, text2, bills[source].name, name, "deposit", trans.trans_id)
                exports['ks-pank']:addAccountMoney(bills[source].job, amount)
            else
                exports['ks-management']:AddMoney(bills[source].job, amount)
            end
